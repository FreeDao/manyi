...
<body class="easyui-layout">
	...
	<div data-options="region:'north',border:false" class="header">当前用户${sessionScope.login_session.userName},<a class="exit" href="${ctx}/logout">退出</a></div>
	<div data-options="region:'west',split:true,title:'列表'" style="width:220px">
		...
		<ul class='tree'>
			<c:if test="${ (sessionScope.login_session.role != 6) || test == 1}">
			<li onclick="parentLi(this)"><a href="javascript:void(0)" onclick="showcontent('${ctx}/check/checkGrid')">审核管理</a></li>
			</c:if>
			<c:if test="${ ((sessionScope.login_session.role != 5) && (sessionScope.login_session.role != 6)) || test == 1}">
			...
			<c:if test="${sessionScope.login_session.role == 1 || sessionScope.login_session.role == 2 || sessionScope.login_session.role == 4 || test == 1}">
				<!-- 系统报表  , 经理级别以上的才能访问-->
				<li onclick="parentLi(this)"><a href="javascript:void(0)" onclick="showcontent('${himsServicePath}')">系统报表</a></li>
			</c:if>
		</ul>
		</c:if>
	</div>
