...
<body onresize="resizeGrid();">
        ...
        <%-- <div title="经纪人账号信息" style="padding:10px">
        	...
        	<span>账户状态：${result.disableStr}</span>
        	<c:if test="${result.disable==true}">
        		<a href="javacript:void(0)"  class="easyui-linkbutton" onclick="disabledUser()">启用</a>
        	</c:if>
        	<c:if test="${result.disable==false}">
        		<a href="javacript:void(0)"  class="easyui-linkbutton" onclick="disabledUser()">冻结账户</a>
        	</c:if>
        	<span>身份证号:${result.code}</span>
        	<span>手机号:${result.mobile}</span>
        	<span>推广人数：</span>
        	<span>待支付金额：${result.waitPayBlance }</span>
        	<span>已支付金额：${result.havePayBlance }</span>
        	<span>支付宝账号：${result.alipayAccount }</span>
        ...
        <div title="经纪人账号信息" style="padding:12px">
			<table style="width:100%; border-spacing:  10px;">
				<tr style="width:auto;text-align: center ;" >
					...
					<td style="width:25%"><b>注册邀请人：</b>${result.spreadName}</td>
				</tr>
				
				<tr style="width:auto;text-align: center ;" >
					<td style="width:25%"><b>身份证号：</b>${result.code}</td>
					<td style="width:25%"><b>手机号：</b>${result.mobile}</td>
					<td style="width:25%"><b>注册时间：</b>${result.createTimeStr}</td>
					<td style="width:25%"><b>账户状态：</b>${result.disableStr}
						<c:if test="${result.disable==true}">
        					<a href="javacript:void(0)"  class="easyui-linkbutton" onclick="disabledUser()">冻结账户</a>
        				</c:if>
			        	<c:if test="${result.disable==false}">
				...
				<tr style="width:auto;text-align: center ;" >
					<!-- <td style="width:25%"><b>推广人数：</b></td> -->
					<td style="width:25%"><b>待支付金额：</b>${result.waitPayBlance }</td> 
					<td style="width:25%"><b>已支付金额：</b>${result.havePayBlance }</td>
					<td style="width:25%"><b>支付宝账号：</b>${result.alipayAccount }</td>
				</tr>
    ...
    <div class="easyui-tabs" style="width:auto;height:250px">
        ...
        <div title="支付记录" style="padding:10px">
     ...
     -->
    <div id="disabledUser" style="display:none">
    	<form action="${ctx}/user/disabledUser" method="post" id="disabledUserForm">
    		<input type="hidden" name="uid" value="${result.uid }"/>
    	</form>
    </div>
    <script type="text/javascript">
    	function disabledUser(){
    		$("#disabledUserForm").submit();
    	}
