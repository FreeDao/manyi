...
public class RentServiceImpl extends BaseService implements RentService {
	...
	public List<HouseEntity> indexList(){
		String jpql="select s,h,e,town,city from HouseResource s,Residence h,Estate e,Town town,City city where "+
				"  s.houseState in ( "+EntityUtils.HouseStateEnum.RENT.getValue()+","+EntityUtils.HouseStateEnum.RENTANDSELL.getValue()+
		...
		if(lists != null && lists.size()>0){
			for (Object[] info : lists) {
				...
				entity.setPrice(hr.getRentPrice());
		...
		log.info("RentServiceImpl indexList : {} ",JSONArray.fromObject(entitys).toString());
		return entitys;
	}
