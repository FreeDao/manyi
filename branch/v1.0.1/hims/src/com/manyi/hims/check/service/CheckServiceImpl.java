...
public class CheckServiceImpl extends BaseService implements CheckService {
	...
	public PageResponse<CheckRes> checktaskList(CheckReq req) {
		...
		String count_sql  ="select count(1)  from HouseResourceHistory hr join House h on h.houseId = hr.houseId join Area e on "
				+" e.areaId = h.estateId join Area town on town.areaId = e.parentId join Area city on city.areaId = town.parentId "
				+"left join User user on user.uid = hr.userId left join Employee emp on emp.employeeId = hr.operatorId  "
				+"where  ( hr.status in (1,2,3,4)  and hr.checkNum = 0 ) ";//审核成功 . 审核失败. 审核中
		String sql  ="select hr.historyId , city.areaId areaId, city.name areaName, town.areaId townid, town.name townNam,e.areaId estateId , e.name estateName,e.road, "
				...
				+" on e.areaId = h.estateId join Area town on town.areaId = e.parentId join Area city on city.areaId = town.parentId  left join User user "
				+" on user.uid = hr.userId left join Employee emp on emp.employeeId = hr.operatorId  "
				+" where ( hr.status in (1,2,3,4)  and hr.checkNum = 0  ) ";
		...
		if (total > 0) {
			...
			if (objs != null && objs.size() > 0) {
				for (Object[] row : objs) {
					...
					check.setAreaId(Integer.parseInt(row[1]+""));
					check.setAreaName(row[2]+"");
					check.setTownId(Integer.parseInt(row[3]+""));
					check.setTownName(row[4]+"");
					check.setEstateId(Integer.parseInt(row[5]+""));
					check.setEstateName(row[6] + (row[7] == null ? "" : row[7]+""));
					check.setBuiling(row[8]+"");
					check.setRoom(row[9]+"");
					check.setPublishId(Integer.parseInt(row[10] == null ? "0" : row[10]+""));
					check.setPublishName(row[11] == null ? "-": row[11]+"");
					check.setPublishDate((Date) row[12]);
					check.setOperServiceId(row[13] == null ? 0 : Integer.parseInt(row[13]+""));
					check.setOperServiceName(row[14] == null ? "-" : row[14]+"");
					check.setOperServiceState(row[15] == null ? 0 :Integer.parseInt(row[15]+""));
					
					int actionType = Integer.parseInt(row[16]+"");//1发布，2，改盘，3举报，4轮询，5抽查 
					int houseState = Integer.parseInt(row[17]+"");//1出租，2出售，3即租又售，4即不租也不售
					
					int houseId = Integer.parseInt(row[18]+"");
