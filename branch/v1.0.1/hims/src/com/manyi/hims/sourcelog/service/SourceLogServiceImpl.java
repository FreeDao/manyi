...
public class SourceLogServiceImpl extends BaseService implements SourceLogService {
	...
	public List<SourceLogResponse> findPageAddAreaLog(int userId, int start, int end, Long currTime) {
		...
		String jqpl  ="from EstateHistory e where e.userId = ? and e.createTime < ? order by e.createTime desc ";//审核成功 . 审核失败. 审核中
		Query query = this.getEntityManager().createQuery(jqpl);
		query.setParameter(1, userId);
		query.setParameter(2, new Date(currTime));
	...
	public UpdateDiscResponse checkCanUpdateDisc(int estateId, String building, String room) {
		...
		cq_houst.where(cb.and(cb.equal(root_houst.get(House_.estateId), sube.getAreaId())),
		...
		if (house == null) {
			...
			Residence houseRes= new Residence();
			houseRes.setEstateId(sube.getAreaId());
			houseRes.setSerialCode(serialCode);
			houseRes.setBuilding(building);
			houseRes.setRoom(room);
			houseRes.setLivingRoomSum(0);
			houseRes.setBedroomSum(0);
			houseRes.setBalconySum(0);
			houseRes.setWcSum(0);
			houseRes.setFloor(0);
			houseRes.setCoveredArea(BigDecimal.valueOf(0));
			houseRes.setHouseRightId(0);
			houseRes.setSpaceArea(BigDecimal.valueOf(0));
			houseRes.setMainOwnerId(0);
			getEntityManager().persist(houseRes);
			this.getEntityManager().flush();
			
			log.debug("该房源未出租或出售 新增的房子 不可以改盘  Residence {}",ReflectionToStringBuilder.toString(houseRes));
