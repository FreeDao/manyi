	...
	*/
	...
	function doSearch(){
		...
		var data_from = $('#mainfrom').datagrid({
			...
			columns : [ [
			          {field : 'pk',title : '小区编号',checkbox : true,width:$(this).width() * 0.03}//显示一个checkbox
					, {field : 'estateId',title : '小区ID',align : 'center',resizable : true,hidden : false,editor:'text',sortable : false,width:$(this).width() * 0.03}
					...
					, {field : 'opt',  title : '操作',align : 'center',resizable : true,hidden : false,sortable : false ,width:$(this).width() * 0.1,
						formatter : function(value,row,index){
			               /*  if (row.editing){
							 ...
							 var e = '<a href="${ctx}/estate/editEstateShow?areaId='+row.estateId+'" target="blank">查看详情</a> ';
			                 return e;
			            }
					  }
			] ],
		...
		function gotoEditEstate(row){
			//row.estateId
			window.location.href="${ctx}/estate/editEstateShow?areaId = "+row.estateId;
		}
