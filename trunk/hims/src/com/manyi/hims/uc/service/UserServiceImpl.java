...
import com.manyi.hims.entity.Recommend;
...
public class UserServiceImpl extends BaseService implements UserService {
	...
	public String findMsgCode(String mobile) {
		...
		try {
			int status = InfoUtils.sendSMS(mobile, content);
			
			addVerifyCode(mobile, status,num);
			log.info("注册-->短信返回状态 : "+status);
			if (status != 1) {
				throw new LeoFault(UcConst.UC_ERROR100017);
			}
		} catch (Exception e) {
			//网络不通
			addVerifyCode(mobile, -2,num);
			e.printStackTrace();
			// 注册验证码发送失败
			throw new LeoFault(UcConst.UC_ERROR100017);
		}
	...
	public void sendMail(Date date,final String content,final String mobile){
	 ...
	 * 保存经纪人手机号码到中介库
	 */
	public void addIntermediary(final String mobile){
		executor.submit(new Runnable() {
			@Override
			public void run() {
				// TODO Auto-generated method stub
//				commonService.addIntermediary(mobile);
			}
		});
...
//	}
	...
	public int regist(RegistRequest req, String sessionCheckMsg) {
		...
		if (req.getCityId() == 0) {
			// 1000051;// 请选择城市
			throw new LeoFault(UcConst.UC_EMPTY_REGIST_1000051);
		}
		if (req.getAreaId() == 0) {
			// 1000052;// 请选择工作区域
			throw new LeoFault(UcConst.UC_EMPTY_REGIST_1000052);
		}
		if (req.getTownId() == 0) {
			// 1000052;// 请选择工作区域
			throw new LeoFault(UcConst.UC_EMPTY_REGIST_1000052);
		...
		try {
			...
			FileUtils.deleteQuietly(req.getCardFile());
			FileUtils.deleteQuietly(req.getCodeFile());
		...
		Recommend recommend = null;
		 if(StringUtils.isBlank(req.getSpreadName())){
			String jpql = "from Recommend re where re.recommendTel =? order by re.createTime asc";
			Query query = getEntityManager().createQuery(jpql);
			query.setParameter(1, req.getMobile());
		...
		user.setRealName(req.getRealName());
		if(recommend != null){
			user.setSpreadName(recommend.getUserTel());
		}else{
			user.setSpreadName(req.getSpreadName());
		}
...
//		addIntermediary(req.getMobile());
	...
	public long findLoginPwd(String phone) {
		...
		try {
			int inputLine = InfoUtils.sendSMS(phone, Global.registBeforeV + random + Global.registAfterV);
			if (inputLine !=1) {
				throw new LeoFault(UcConst.UC_ERROR100017);
			}
		} catch (Exception e) {
...
//		houseResourceService.publicAudit(houseId);
	...
	public FailedDetailRes getFailedDetail(String mobile) {
		...
		if(userList != null && userList.size()>0){
			...
			if(user.getState() == EntityUtils.StatusEnum.FAILD.getValue()){
				...
				Province province = this.getEntityManager().find(Province.class, user.getCityId());
				res.setCityId(province.getAreaId());
				res.setCityName(province.getName());
				
				City city = this.getEntityManager().find(City.class, user.getAreaId());
				res.setAreaId(city.getAreaId());
				res.setAreaName(city.getName());
				
				if (user.getTownId() != 0) {
					Town town = this.getEntityManager().find(Town.class, user.getTownId());
				...
				res.setCodeFile(new File(user.getCodeUrl()));
				res.setCardFile(new File(user.getCardUrl()));
			}else{
				// 100020;//帐号处于非审核失败状态
	...
	@Override
	public void recommend(int uid, String userTel, String recommendTel) {
		if(uid==0){
			throw new LeoFault(UcConst.UC_ERROR100029);
		}
		if(StringUtils.isBlank(userTel) || !DataUtil.checkMobile(userTel)){
			throw new LeoFault(UcConst.UC_ERROR100029);	
		}
		String[] phone_tmp = recommendTel.split(",");
		for (int i = 0; i < phone_tmp.length; i++) {
			if(StringUtils.isBlank(phone_tmp[i])){
				throw new LeoFault(UcConst.UC_ERROR100029);
			}else if(StringUtils.isNotBlank(phone_tmp[i])&&!DataUtil.checkMobile(phone_tmp[i].replace(" ", ""))){
				throw new LeoFault(UcConst.UC_ERROR100029);
			}
		}
		for (int i = 0; i < phone_tmp.length; i++) {
			...
			recommend.setUserTel(userTel);
			recommend.setRecommendTel(phone_tmp[i].replace(" ", ""));
			getEntityManager().persist(recommend);
		}
	}
	/**
	 * 保存短信验证码
	...
	public void addVerifyCode(String mobile,int status,String num){
		String jpql = "from VerifyCode verCode where verCode.mobile=?";
		Query query = getEntityManager().createQuery(jpql);
		query.setParameter(1, mobile);
		List<VerifyCode> verifyCodeList = query.getResultList();
		if(verifyCodeList==null || verifyCodeList.size()<=0){
			VerifyCode code = new VerifyCode();
			code.setMobile(mobile);
			code.setStatus(status);
			code.setVerifyCode(num);
			getEntityManager().persist(code);
		}else{
			VerifyCode code = verifyCodeList.get(0);
			code.setMobile(mobile);
			code.setStatus(status);
			code.setVerifyCode(num);
