...
public class UserRestController extends RestController {
	...
	public DeferredResult<Response> userLogin(HttpSession session,
		...
		final UserInfo user = getUserService().userLogin(params.getLoginName(), params.getPassword());
	...
	public DeferredResult<Response> addEstate(@RequestBody AddEstateRequest params) {
		...
		response.setErrorCode(100030);
		response.setMessage("新增小区功能暂时关闭");
...
//		dr.setResult(new Response());
	 ...
	 * 
	 */
	@RequestMapping(value = "/recommend.rest", produces = "application/json")
	@ResponseBody
	public DeferredResult<Response> recommend(@RequestBody RecommendRequest params) {
		userService.recommend(params.getUid(), params.getUserTel(), params.getRecommendTel());
		DeferredResult<Response> dr = new DeferredResult<Response>();
		dr.setResult(new Response());
		return dr;
	}
