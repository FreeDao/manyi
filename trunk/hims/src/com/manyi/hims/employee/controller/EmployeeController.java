...
import com.manyi.hims.employee.service.EmployeeService.AreaResponse;
...
public class EmployeeController extends RestController {
	...
	public DeferredResult<Response> login(@RequestBody EmployeeModel emp) {
		return CommonUtils.deferredResult(employeeService.login(emp));
	}
	 ...
	 *
	 */
	@RequestMapping(value = "/getProvince.rest", produces="application/json")
	@ResponseBody
	public DeferredResult<Response> getProvinve() {
		Result result = this.employeeService.getProvinde();
		DeferredResult<Response> dr = new DeferredResult<Response>();
		dr.setResult(result);
		return dr;
	}
	 ...
	 *
	 */
	@RequestMapping(value = "/getCity.rest", produces="application/json")
	@ResponseBody
	public DeferredResult<Response> getCity(@RequestBody getCityRequest pars) {
		Result result = this.employeeService.getCityByParentId(pars.getParentId());
		DeferredResult<Response> dr = new DeferredResult<Response>();
		dr.setResult(result);
		return dr;
	}
