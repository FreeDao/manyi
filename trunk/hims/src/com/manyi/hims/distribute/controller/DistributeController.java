...
public class DistributeController {
	...
	@ResponseBody
	public Response distribute(@RequestBody DistributeRequest distributeRequest){
		return distributeService.distribute(distributeRequest);
	}
	...
	@ResponseBody
	public Response autoDistribute(@RequestBody EmployeeModel emp){
		Response response = null;
		
		System.out.println("hims empId " + emp.getId());
		if (emp .getId() <= 0) {
			return new Response(1586004,"error employeeId");
		}
		response = distributeService.getDistributedHouseResource(emp.getId());
		if(response == null){
			response = distributeService.autoDistribute(emp);
		}
