...
public class ExcelUtils {
	 ...
	 * @return
	 */
	public static <T> boolean defExportExcel(HttpServletResponse response, List<T> list, String xlsName, String... headTitle) {
		...
		return exportExcel(response, list, xlsName, new IExcel<T>() {
			...
			public void initHSSRow(List<T> list, HSSFSheet sheet) {
				...
				for (int j = 0; j < list.size(); j++) {
					...
					Iterator<String> ir = fieldValMap.keySet().iterator();
					int i = 0;
					while (ir.hasNext()) {
	...
	public static <T> boolean exportExcel(HttpServletResponse response, List<T> list, String name, IExcel<T> iExcel, String... headTitle) {
		...
		String xlsName = name + (new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(new Date()));
		log.info(xlsName);
		// 产生工作簿对象
