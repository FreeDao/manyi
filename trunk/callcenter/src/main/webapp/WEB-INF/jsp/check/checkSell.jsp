...
<body onresize="resizeGrid();" style="margin:0px;padding:0px">
		...
		<div style="padding:12px">
			<table style="width:100%; border-spacing:  10px;">
				<tr style="width:auto;text-align: center ;" >
					<td style="width:25%"><strong>${css.house.subEstateName } - ${css.house.subEstateStr } - <span title="栋座号"><c:if test="${css.house.building == '0'}">独栋 </c:if><c:if test="${css.house.building != '0'}">${css.house.building } </c:if></span>- <span title="室号">${css.house.room }</span></strong></td>
				...
				<tr style="width:auto;text-align: center;" >
					...
					<td style="width:25%"><span><b>门店名称：</b>${css.houseResource.doorName }</span></td>
				</tr>
				<tr style="width:auto;text-align: center;" >
	...
	<div style="width:auto;height:auto">
		...
		<div  style="width:auto;height:auto;padding:20px 60px">
			<div>
			<c:forEach items="${css.contactResponses }" var="host" varStatus="status">
				<div class="note-word">提示语：请问你是<span style="font:bold;font-size:1.2em;color: red;">${css.house.subEstateName }  ${css.house.subEstateStr} <c:if test="${css.house.building != '0'}">- ${css.house.building }栋 </c:if>- ${css.house.room }室</span>的房主<span style="font:bold;font-size:1.2em;color: red;">${host.hostName} </span> 吗？ 请问你的房子是<span style="font:bold;font-size:1.2em;color: red;"><c:if test="${css.houseResource.actionType == 2}">${css.houseResource.stateReasonStr }</c:if><c:if test="${css.houseResource.actionType != 2}">${css.houseResource.houseStateStr }</c:if></span>么？</div>
			</c:forEach>
				...
				<table style="width:100%">
					<tr style="width:auto;">
						<td style="width:50%">
							...
							<c:if test="${css.houseResource.actionType == 1 && css.houseResource.houseState == 1 }">
							<!-- 出租 的时候. 显示  可入住时间  到期时间  -->
							<tr>
								<td class="lbl">可入住时间：</td>
								<td>
									<input class="easyui-datebox txt" editable="false" name="rentFreeDate" data-options="required:false,showSeconds:true" style="width:100px">
								</td>
							</tr>
							<tr>
								<td class="lbl">到期时间：</td>
								<td>
									<input class="easyui-datebox txt" editable="false" name="rentTermDate" data-options="required:false,showSeconds:true" style="width:100px">
								</td>
							</tr>
							</c:if>
							</table>
			...
			<div  id="antiCheat">
					<c:forEach items="${css.contactResponses }" var="host" varStatus="status">
						<c:if test="${ host.status == 0}"><input name="mobile" type="hidden" value="${host.hostMobile }"  /></c:if>
					</c:forEach>
					<input name="estateId" type="hidden" value="${css.house.estateId }"  />
			</div>
		</div>
		</c:if>
...
function submitCheckForm(status){
	...
	if(text1 != null && text2 != null && text3 != null) {
		if( /^\d+$/.test(text1) && /^\d+$/.test(text2) && /^\d+$/.test(text3)) {
			if(text1 > 9 || text2 > 9 || text3 > 9) {
				$.messager.alert('温馨提示', "房型数据不能超过9","warning");
				return;
			}
		}else {
	...
	$('#submitCheck-form').form({
		success:function(data){
			...
			if(data=="success"){
				...
				window.location.href="${ctx}/check/cs/startWork?t=" + timestamp;
			}else {
...
function antiCheatAjax(){
	var mobile = $("#antiCheat input[name='mobile']").val();
	var estateId = $("#antiCheat input[name='estateId']").val();
	if(mobile != null && mobile != '' && mobile !='0' && estateId != null && estateId != '' && estateId !='0') {
		$.get("${ctx}/check/cs/antiCheat", {'mobile':mobile,'estateId':estateId},function(data){
			$("#antiCheat").append(data);
		});
	}else {
	}
