...
(function($) {
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function() {
            ...
            var _id = null;
            _.textwrapper.mouseover(function() {
                clearTimeout(_id)
            });
            _.textwrapper.mouseout(function() {
                _id = setTimeout(function() {
                    _.selectBox.hide()
                },
                300)
            });
            _.selectBox.hover(function() {
                clearTimeout(_id)
            });
            ...
            if (!A.selectBoxHeight && B < 8) {
                A.selectBoxHeight = B * 30
            };
            if (A.selectBoxHeight) {
                _.selectBox.height(A.selectBoxHeight)
            };
            _.bulidContent();
            _.set(A);
    ...
    $.ligerui.DialogImagePath = "../../lib/ligerUI/skins/Aqua/css/img/win/";
    function A(A) {
    ...
    $.ligerDefaults.DialogString = {
        ...
        no: "\u004e\u006f",
        cancel: "Cancel",
        waittingMessage: "Operating,Please Wait......"
    };
        ...
        _render: function() {
            ...
            if (B.fields && B.fields.length) {
                ...
                $(B.fields).each(function(A, $) {
                    var B = $.name || $.id ,_value = "";
                    if (!B) return;
                    if ($.type == "hidden") {
                    	if ($.options && $.options.value){
                    		_value = $.options.value;
                    	}
                        D.push("<input type=\"hidden\" id=\"" + B + "\" name=\"" + B + "\" value=\"" + _value + "\" />");
                        return
                    }
...
(function($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        onRowDragFun: function(rows, targetRow, nearRow) {
            return true
        },
        onRowDragDrop: null,
        rowHeight: 22,
    ...
    $.ligerDefaults.GridString = {
        errorMessage: "error",
        pageStatMessage: "从 {from}条 至 {to}条\uff0c 总数 {total}条 每页:{pagesize}条",
        pageTextMessage: "Page",
        loadingMessage: "loading...",
        findTextMessage: "Search",
        noRecordMessage: "contentType MAPPING DATA",
        isContinueByDataChanged: "The data has changed, if you continue to lose data, whether to continue?",
    ...
    $.ligerDefaults.Grid.editors["int"] = $.ligerDefaults.Grid.editors["float"] = $.ligerDefaults.Grid.editors["spinner"] = {
        create: function(B, _) {
            ...
            var D = {
                ...
                _type: C.editor.type == "float" ? "float": "int"
            };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function() {
            ...
            C.push("                <div class='l-bar-separator'></div>");
            C.push("                <div class='l-bar-group'>");
            C.push("                     <div class='l-bar-button l-bar-go'><span></span></div>");
            C.push("                </div>");
            C.push("                <div class='l-bar-separator'></div>");
        ...
        _resize: function(_$) {
        	 var $ = this,A = this.options;
        	clearTimeout(window.t);
        	window.t = setTimeout(function(){
        		$._onResize();var width = $.gridview2.width();
        		jQuery($.columns).each(function(i, B) {
        			if (B._width < 50){return;}
                    var _;
                    if (B.width) _ = B.width;
                    else if (A.columnWidth) _ = A.columnWidth;
                    if (typeof(_) == "string" && _.indexOf("%") > 0) _ = parseInt(parseInt(_) * 0.01 * (width - A.columns.length));
         			if (_){
         				$.setColumnWidth(i,_);
         			}
        		});
        ...
        setColumnWidth: function(_, G) {
            ...
            $(B).css("width", G).find("> div.l-grid-row-cell-inner:first").css("width", G - 16);
        ...
        _isColumnId: function(i) {
        	return (i in this._columns)
        },
        ...
        _getColumnWidth: function(B) {
            ...
            if (A.checkbox){
            	if (typeof(_) == "string" && _.indexOf("%") > 0) B._width = _ = parseInt(parseInt(_) * 0.01 * ($.grid.width()- 29 - $.columns.length));
            } else {
        ...
        _getCellHtml: function(D, B) {
            ...
            C.push(" style = \"width:" + parseInt(B._width - 16) + "px;");
        ...
        _getTreeCellHtml: function(D, G) {
            ...
            B = parseInt(B) || 1;
            for (var H = 1; H < B; H++){
            	if (!G.__hasChildren&&p.lGridTreeNoChild) {
	            	_ += "<div class='l-grid-tree-space l-grid-no-child-space'></div>";
            	} else {
            		_ += "<div class='l-grid-tree-space'></div>";
            	}
            } 
            if (E && F) _ += "<div class='l-grid-tree-space l-grid-tree-link l-grid-tree-link-open'></div>";
            	...
            	if (!G.__hasChildren&&p.lGridTreeNoChild) {
            		_ += "<div class='l-grid-tree-space l-grid-no-child-space l-grid-no-child'></div>";
            	} else {
            		_ += "<div class='l-grid-tree-space'></div>";
            	}
            }
        ...
        _getSrcElementByEvent: function(B) {
            ...
            if (C.toolbar) {
                ...
                C.input = $(".pcontrol input", _.toolbar);
            }
            return C
        },
        _setEvent: function() {
            ...
            function($) {
                delete _.ctrlKey;
                var ev =_._getSrcElementByEvent($);
                var input = ($.target || $.srcElement);
                if (ev.input && ev.input[0] == input) {
            ...
            if ($.fn.ligerDrag && A.rowDraggable) {
            	var _i = true,_y = true;
                A.rowDraggableTip = A.rowDraggableTip || "鼠标按下移动排序！";
                var verifyTip = $('<div class="l-verify-tip"><div class="l-verify-tip-corner"></div><div class="l-verify-tip-content" style="width: 110px;">' + A.rowDraggableTip + '</div></div>').hide();
                $(_.body).append(verifyTip);
                $(_.gridbody).mousemove(function(event) {
                    var E = _._getSrcElementByEvent(event);
                    if (E.cell == null) {
                        return
                    }
                    if (_i){
                    ...
                    var top = _.gridbody.offset().top - _.gridheader.height() + 5;
                    verifyTip.css({
                        left: event.clientX,
                        top: event.clientY - top
                    });
                });
                $(_.gridbody).mouseout(function(event) {
                    verifyTip.hide();
                });
                var _self = this;
                _.rowDroptip = $("<div class='l-drag-rowdroptip' style='display:none'></div>").appendTo("body");
                _.gridbody.ligerDrag({
                    revert: true,
                    animate: false,
                    proxyX: 0,
                    proxyY: 0,
...
//                            var D = $('<div class="l-drag-proxy l-frozen" style="display:none;overflow:hidden"></div>').appendTo("body");
//                            D.width(_.gridview2.width()+3);
//                            $(D).append('<div class="l-grid1" style="width: 30px;"><table cellspacing="0" cellpadding="0" class="l-grid-header-table"><tbody></tbody></table></div>');
//                            $(D).append('<div class="l-grid2" style="left: 30px;"><table cellspacing="0" cellpadding="0" class="l-grid-header-table"><tbody></tbody></table></div>');
//                            var _A = _self,_F = _A.getRow(E.row),G = _F["__id"],B = _A.getRowObj(G),_c = _A.getRowObj(G, true);
//                            $(".l-grid1 tbody",D).append($(_c).clone(true));
//                            $(".l-grid2 tbody",D).append($(B).clone(true));
                            return D
                        }
                    },
                    onRevert: function() {
                    ...
                    onStartDrag: function(C, $) {
                        ...
                        if (!jQuery(D.row).hasClass("l-selected")) {
                            _.select(D.row);
                            window.row = D.row
                        }
                        if (D.treelink) return false;
                        if (!D.cell || !D.data || D.checkbox) return false;
                        var A = D.cell.id.split("|"),
                        B = _._columns[A[A.length - 1]];
                        if (D.rownumberscell || D.detailcell || true || B == _.columns[0]) {
                    ...
                    onStopDrag: function(D, C) {
                        ...
                        if (_.rowDropIn != -1) {
                            if (A.tree) {
                                ...
                                if (A.onRowDragFun(H, B, F, _.rows) !== false) {
                        ...
                        if (window.row) {
                            _.unselect(window.row);
                            window.row = null
                        }
                    }
                });
                _.f.gridbody.ligerDrag({
                    revert: true,
                    animate: false,
                    proxyX: 0,
                    proxyY: 0,
        ...
        _onContextmenu: function(A) {
            ...
            if (D.row) {
                ...
                if (_.hasBind("contextmenu")) return;
                _.trigger("contextmenu", [{
                    data: D.data,
        ...
        _onClick: function(L) {
            ...
            if (I.checkboxall) {
                ...
                if (K.trigger("toGo", [K.element]) == false) return false;
                K.changePage("input")
            }
        },
        select: function(C) {
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        moveToPrevTabItem: function() {
            ...
            $("li", _.tab.links).each(function(C, _) {
                var A = -1 * B;
                if (C > 0) {
                    A = parseInt(D[C - 1]) + $(this).prev().width() + 2
                }
                D.push(A)
            });
        ...
        moveToNextTabItem: function() {
            ...
            for (var I = H.length - 1; I >= 0; I--) {
                var F = J - B + C + 2;
                if (I != H.length - 1) {
                    F = parseInt(G[H.length - 2 - I]) - $(H[I + 1]).width() - 2
                }
                G.push(F)
            }
            var lw = parseInt(_.tab.links.width());
            var E = -1 * parseInt(_.tab.links.ul.css("left"));
            for (var D = 1; D < G.length; D++) {
                if (G[D] <= E && G[D - 1] > E) {
                    var _y = parseInt(G[D - 1]);
                    if (Math.abs(E) - Math.abs(G[D]) >= C) {
                        _y = parseInt(G[D] - 2)
                    }
                    _.tab.links.ul.animate({
                        left: -1 * _y
        ...
        setTabIsShow: function() {
            var left = $(".l-tab-links-left", this.tab.links);
            if (left.length > 0) {
                var _ = this,
                w = 0,
                ww = 0,
                ...
                var l = Math.abs(parseInt(_.tab.links.ul.css("left")));
                var lw = parseInt(_.tab.links.width());
                if (D[w + 1] < l + B || D[w] + ww > l + lw - B) {
                    if (D[w + 1] < l + B) {
                        _.tab.links.ul.animate({
                            left: -1 * parseInt(D[w])
                        })
    ...
    $.fn.ligerTip = function(_) {
        this.each(function() {
            ...
            if (A.auto || _ == undefined) {
                ...
                function() {
                    A.x = $(this).offset().left + $(this).width() + (A.distanceX || 0) + (A.speedX || 0);
                    A.y = $(this).offset().top + (A.distanceY || 0) + (A.speedY || 0);
                    $.ligerTip(A)
                }).bind("mouseout.tip", 
                function() {
                ...
                A.x = A.x + (A.speedX || 0) || 0;
                A.y = A.y + (A.speedY || 0) || 0;
                $.ligerTip(A)
            }
        });
