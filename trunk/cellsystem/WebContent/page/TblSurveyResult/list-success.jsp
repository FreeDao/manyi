			...
			var crGrid = {
				...
				init : function() {
					...
					this.grid = jQuery(self.main).ligerGrid({ 
			         	columns: [
							{display:"问卷名称",name:"dname",width:"50%",align:'left'},
							{display:"参加人数",name:"dparticipants",width:"50%"}
					   ],
			           dataAction: 'server', 
			           pageSize: 15, 
			           toolbar: {},
			           ...
			           onDblClickRow:function(data, rowindex, rowobj){
							top.framework.addTab(LG.getPageMenuNo()+'_'+data.did+'_view','查看调查统计', 'TblSurveyResult/view.do?id='+data.did);
						}
			      });
			      	self.search();
					if((permissionValue&1)==1){
					      this.loadToolbar(self.grid, function(e){self.toolbar(e)},menuno);
					}
				},
				...
				search : function() {
					jQuery(this.formsearch).ligerForm({
						...
						labelAlign: "right", 
						fields:[
								{display:"问卷名称",name:"query.dname",newline:true}
							]
                    });
				...
				view : function() {
	                ...
	                top.framework.addTab(LG.getPageMenuNo()+'_'+selecteds[0].did+'_view','查看调查统计', 'TblSurveyResult/view.do?id=' + selecteds[0].did);
				},
				exportdata : function() {
					var selecteds = this.grid.getSelecteds();
	                if (selecteds.length == 0) { LG.tip(LG.INFO.ONE); return }
	                if (selecteds.length>1) { LG.tip(LG.INFO.TWO); return }
					window.location.href="<%=path%>/TblSurveyResult/exportdata.do?id="+ selecteds[0].did;
				},
				reload : function() {
					this.grid.loadData();
				}
