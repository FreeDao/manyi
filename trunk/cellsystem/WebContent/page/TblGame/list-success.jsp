	...
	<body>
		...
		<script type="text/javascript">
			var crGrid = {
				...
				init : function() {
					...
					this.grid = jQuery(self.main).ligerGrid({ 
			         	columns: [
							...
							,{display:"资费(元)",width:"50%",name:"dvalue"}							
					   ],
			           dataAction: 'server', 
			           pageSize: 15, 
			           toolbar: {},
			           ...
			           onDblClickRow:function(data, rowindex, rowobj){
							top.framework.addTab(LG.getPageMenuNo()+'_'+data.did+'_view', '查看资费策略', 'TblGame/view.do?id='+data.did);
						}
			      });
			     self.search();
				...
				add : function() {
					top.framework.addTab(LG.getChildMenuNo(), '新增资费策略', '<%=path%>/TblGame/add.do?menuno='+LG.getPageMenuNo());
				},
				view : function() {
	                ...
	                top.framework.addTab(LG.getPageMenuNo()+'_'+selecteds[0].did+'_view', '查看资费策略', '<%=path%>/TblGame/view.do?menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did);
				},
				modify : function() {
					var selecteds = this.grid.getSelecteds();
					if (selecteds.length == 0) {LG.tip(LG.INFO.ONE);return}
					if (selecteds.length > 1) {LG.tip(LG.INFO.TWO);return}
	                top.framework.addTab(LG.getPageMenuNo()+'_'+selecteds[0].did+'_modify', '修改资费策略', '<%=path%>/TblGame/modify.do?id=' + selecteds[0].did+"&menuno="+LG.getPageMenuNo());
				},
				off:function(){
