		...
		*/
        ...
        function f_onAfterEdit(e)
        {
            if (e.column.name == "dsequence")
            {
            	...
            	if(data!=undefined){
            	if(data[0]!=undefined){
	            	var dt={"tblVodcategory.did":e.record.did,"tblVodcategory.dsequence":e.value,"updateSequence":"1"};
		            	$.ajax({
							async:false,
							data:dt,
							url:"<%=path%>/TblVodcategory/update.do",
							dataType:"json",
			...
			var crGrid = {
				...
				init : function() {
					...
					this.grid = jQuery(self.main).ligerGrid({
			         	columns: [
							...
							{display:"创建时间",width:"35%",name:"dcreatetime"},
			           ...
			           */
				       ...
				       onDblClickRow:function(data, rowindex, rowobj){
							top.framework.addTab(LG.getPageMenuNo()+'_'+data.did+'_view','查看栏目信息', 'TblVodcategory/view.do?id='+data.did);
						}
			      });
			      //self.search();
				...
				add : function() {
					top.framework.addTab(LG.getChildMenuNo(), '新增影视栏目信息', 'TblVodcategory/add.do?menuno='+LG.getPageMenuNo());
				},
				view : function() {
	                ...
	                top.framework.addTab(LG.getPageMenuNo()+'_'+selecteds[0].did+'_view','查看影视栏目信息', 'TblVodcategory/view.do?IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did);
				},
				modify : function() {
					var selecteds = this.grid.getSelecteds();
					if (selecteds.length == 0) {LG.tip(LG.INFO.ONE);return}
					if (selecteds.length > 1) {LG.tip(LG.INFO.TWO);return}
	                top.framework.addTab(LG.getPageMenuNo()+'_'+selecteds[0].did+'_modify','修改影视栏目信息', 'TblVodcategory/modify.do?IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did);
				},
				off:function(){
