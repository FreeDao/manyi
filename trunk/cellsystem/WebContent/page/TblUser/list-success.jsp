...
String deptId = request.getAttribute("deptId")+"";
%>
	...
	<body>
		...
		<script type="text/javascript">
			var crGrid = {
				...
				init : function() {
					...
					this.grid = jQuery(self.main).ligerGrid({ 
			         	columns: [
						{display:"用户账号",name:"dusername",width:"40%",isSort : false,align:'left'},
						{display:"员工名称",name:"demployeename",isSort : false,width:"20%"},
						{display:"密码",name:"dpassword",isSort : false,width:"20%"},
						//{display:"计费账号",name:"daccount",width:"25%"},
						{display:"状态",name:"dstatus",width:"20%",isSort : false,render: LG.status}
					   ],
			           dataAction: 'server', 
			           pageSize: 15, 
			           toolbar: {},
			           ...
			           url:  'getData.do?deptId=${sessionScope.deptId}',
				...
				search : function() {
					jQuery(this.formsearch).ligerForm({
						...
						fields:[
								...
								{display:"员工名称",name:"query.demployeename",newline:false},
								{type:"hidden",name:"query.did",options:{value:1}},
				...
				batchSave:function(){
					...
					*/
					...
					$("#result").html("");
					$(".l-custom-ftpWin").empty();
					$("#result").load('<%=path%>/page/TblUser/batchSave.jsp');
			...
			$(document).ready(function(){
				$(document.getElementById("query.did")).parent("li").css("display","none");
			});
		</script>
	</body>
</html>