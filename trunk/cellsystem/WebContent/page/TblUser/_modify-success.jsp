...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			var employeeList=<%=request.getAttribute("employeeList")%>;
			if(employeeList==null){
				employeeList={"rows" : [],"total" : 0};
			}
		</script>
	</head>
	<body>
		...
		<script type="text/javascript">
			...
			var crForm = {
				...
				init : function() {
				    ...
				    this.mainform.ligerForm({
				        ...
				        fields : [
							...
							{display:"用户账号",name:"tblUser.dusername",space:LG.spaceWidthB,newline:false,group:"基本信息",groupicon:groupicon,options:{disabled:true}},
							{display:"用户密码",name:"tblUser.dpassword",newline:false,validate:{onlyGraphemeOrNum:[6,16]},initValue:123456},
							{display:"付费类型",name:"dpaytype",space:LG.spaceWidthB,newline:true,type:"select",comboboxName:"dpaytype",options:{valueFieldID:"tblUser.dpaytype",data:PayType}},
							//{display:"计费账号",name:"tblUser.daccount",newline:false,validate:{onlyGraphemeOrNum:[5,40]},options:{disabled:true}},														
							...
							{display:"用户状态",name:"dstatus",newline:false,type:"select",comboboxName:"dstatus",options:{valueFieldID:"tblUser.dstatus",selectBoxHeight:60,data:LG.statusTypeData}}							
						],
				   ...
				   this.grid = jQuery("#maingrid").ligerGrid({ 
			           ...
			           onBeforeSubmitEdit:function(e){
						...
						var isOnly=true;
						if(this.data.rows[record.__index].employeeno!= undefined && this.data.rows[record.__index].employeeno!=employeeno){
						$.ajax({
							cache : false,
							async : false,
				...
				save : function() {
					var employeeArray=[];
					var dd=this.grid.rows;
					if(dd!=null&&dd!=undefined&&dd.length>0){
						var len=dd.length;
						for(var i=0;i<len;i++){
							if(dd[i].__status!="delete"){
								if(dd[i]._editing){
				...
				loadForm : function() {
					LG.loadForm(this.mainform, {url:"loadModify.do?id="+id,preID:'tblUser\\.'},function(){});
				},
				addNewRow : function() {
