...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<div class="l-group l-group-hasicon"><img src="<%=path%>/static/xcloud/css/img/icon/communication.gif" /><span>基本信息</span> >> <s:property value="#attr.categoryName"/></div>
			<form id="mainform" method="post" action="save.do" >
		
			<table id="detail" class="detail-tableSkin detail-tableSkinB">
				<tr>
					<th>问卷名称：</th>
					<td style="width: 380px">
						<input class="BB" name="tblQuestionexam.dname"  validate="{'required':true}"/>
			...
			*/
			function addAn(num){
				...
				if(codeAscii<"Z".charCodeAt()){
					...
					//var str= '<div class="num" style="margin-right:100px;">&nbsp;<span>'+txt+'.&nbsp;</span>&nbsp;&nbsp;<input class="AA" name="dcontent'+txt+'" validate="{\'required\':true}" onchange="changeAn(this)" /><div class="l-tab-close" onclick="deleteAn('+num+')"></div> &nbsp;<input type="hidden" name="dcontent" class="dcontent" /></div>';
			...
			function submitE(num){
				...
				$.ajax({
					...
					success:function(data){
						...
						con=eval("["+con+"]");
						var profile={nexistNo:num,isedit:'isedit',showNo:sNo,data:{did:temp.did,dtype:temp.dtype,dcontent:con,dtitle:temp.dtitle}};
				...
				save : function() {
					...
					mainFrm.ajaxForm({
						...
						success:function(data){
							...
							//问卷添加成功之后, 需要把选项入库, 或者 是更新选项的考卷的ID	
							var frms=$('#listcontent').find("form");
							var selectids='';
							var sequences='';
							for(var i = 0; i<frms.length; i++){
								...
								var isedit=frm.find("#isedit").val();
								sequences+=frm.find("#sequence").val()+","; // 记录 每道考题的 排序sequence
