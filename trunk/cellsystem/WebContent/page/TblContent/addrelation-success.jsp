	...
	<head>
		...
		<form id="mainform" method="post" action="TblQuestionexam/save.do" >
			<dl class="l-custom-tab">
				<dt><a href="addexistcontent.do?dtype=<%=dtype %>&categoryType=<%=categoryType %>&menuno=<%=menuno %>&contentid=<%=contentid %>">新增内容</a></dt>
				<dd><a href="javascript:void(0);">关联试卷</a></dd>
			</dl>
			<div id="switch-btn" class="switch-btn">
				<input type="radio" value="0" name="switch" id="rbtnl_0" url="" checked="checked"/><label for="rbtnl_0">新建试卷</label>
				<input type="radio" value="1" name="switch" id="rbtnl_1" url="questlist.do?dtype=<%=dtype %>&categoryType=<%=categoryType %>&menuno=<%=menuno %>&contentid=<%=contentid %>"/><label for="rbtnl_1">关联已有试卷</label>
			</div>
			<table id="detail" class="detail-tableSkin detail-tableSkinB">
				<tr>
					...
					<th>时长(分)：</th>
					<td>
						<input class="YY" name="tblQuestionexam.dduration"  validate="{'required':true,'digits':true}"/>
					</td>
				</tr>
				<tr>
					<th>激活时间：</th>
				...
				<tr>
					<th>通过分数：</th>
					<td><input class="YY" name="tblQuestionexam.dpassvalue"  validate="{'required':true,'digits':true}"/> </td>
					<td></td>
					<td></td>
				</tr>	
				<tr>
                ...
                <dd>
                    <table width="90%" class="detail-tableSkin"> 
                        <tr>
                            ...
                            <td>
                                <input class="GG" name="tblExamexercise.dtitle" value="{$T.data.dtitle}" validate="('required':true,'maxlength':64)"/>
                            </td>
                        </tr>
                        <tr>
                            <th  width="9%">类型：</th>
...
<script type='text/javascript'>
		var nexistNo=1,showNo=1 ,currNo = 1;
			...
			function renderInput(){
				$(".AA").ligerTextBox({width:LG.inputWidthA+70});
				$(".GG").ligerTextBox({width:LG.inputWidthB});
				$(".BB").ligerTextBox({width:182});
				$(".YY").ligerTextBox({width:202});
					...
					mainFrm.ajaxForm({
						...
						success:function(data){
							...
							//问卷添加成功
							 ...
							 win.dtype=<%=dtype%>;
							 //alert("win.categoryType:"+win.categoryType);
