        ...
        function f_onAfterEdit(e)
        {
            if (e.column.name == "dsequence")
            {
            	...
            	if(data[0]!=undefined){
	            	...
	            	var dt={"tblContent.did":data[0].did,"tblContent.dsequence":e.value,"updateSequence":"1"};
		            	$.ajax({
							async:false,
							data:dt,
							url:"<%=path%>/TblContent/update.do",
							dataType:"json",
			...
			var crGrid = {
				...
				init : function() {
					...
					this.grid = jQuery(self.main).ligerGrid({ 
			         	columns: [
							...
							{display:"上传时间",name:"duploadtime",width:"16%"},
							{display:"上传人",name:"duploadperson",width:"16%"},
							{display:"状态",name:"dstatus",width:"16%",render: LG.check}
					   ],
					   enabledEdit:true,
			           ...
			           url:  'loadList.do?categoryType=<%=categoryType%>&type=<%=type%>', 
			           //sortName: 'd_id',sortOrder:'desc',
			           width: '98%', height: '100%',heightDiff:-13, checkbox: true
			          ,
			           onDblClickRow:function(data, rowindex, rowobj){
			           var path1='TblContent/view.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + data.did;
						var title="查看内容信息";
						if(<%=dtype%>==1){
							//查看 问卷调查
							path1='TblContent/viewquest.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + data.did;
							title="查看问卷调查";
						}else if(<%=dtype%>==5){
							//查看 在线考试
							path1='TblContent/viewexist.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + data.did;
							title="查看在线考试";
						}else if(<%=dtype%>==15){
							//查看 培训内容, 可以关联考卷
							path1='TblContent/view.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + data.did;
							title="查看培训内容";
						}
				...
				add : function() {
					...
					//首页的内容不能超过4个
					if(<%=dtype%>==0){
						if(this.grid.getData().length>=4){
							LG.tip('首页的内容不可超过四个',null,1000);
							return ;
						}
					}
					...
					$.ajax({
							async:false,
							data:{"query.did":<%=categoryType%>},
							url:"<%=path%>/TblCategory/getData.do",
							dataType:"json",
							success:function(data){
								if(data!=undefined){
					...
					var path1='TblContent/add.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&menuno='+LG.getPageMenuNo();
					var title="新增内容信息";
					if(<%=dtype%>==1){
						//添加 问卷调查
						path1='TblContent/addquest.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&menuno='+LG.getPageMenuNo();
						title="新增问卷调查";
					}else if(<%=dtype%>==5){
						//添加 在线考试
						path1='TblContent/addexist.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&menuno='+LG.getPageMenuNo();
						title="新增在线考试";
					}else if(<%=dtype%>==15){
						//添加 培训内容, 可以关联考卷
						path1='TblContent/addexistcontent.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&menuno='+LG.getPageMenuNo();
						title="新增培训内容";
					}
				...
				view : function() {
					...
					if(<%=dtype%>==1){
						//查看 问卷调查
						path1='TblContent/viewquest.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did;
						title="查看问卷调查";
					}else if(<%=dtype%>==5){
						//查看 在线考试
						path1='TblContent/viewexist.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did;
						title="查看在线考试";
					}else if(<%=dtype%>==15){
						//查看 培训内容, 可以关联考卷
						path1='TblContent/view.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did;
						title="查看培训内容";
					}
				...
				modify : function() {
					...
					if (selecteds.length == 0) {LG.tip(LG.INFO.ONE);return}
					if (selecteds.length > 1) {LG.tip(LG.INFO.TWO);return}
	                  var path1='TblContent/modify.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did;
					var title="修改内容信息";
					if(<%=dtype%>==1){
						//修改 问卷调查
						path1='TblContent/modifyquest.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did;
						title="修改问卷调查";
					}else if(<%=dtype%>==5){
						//修改 在线考试
						path1='TblContent/modifyexist.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did;
						title="修改在线考试";
					}else if(<%=dtype%>==15){
						//修改 培训内容, 可以关联考卷
	                ...
	                top.framework.addTab(LG.getPageMenuNo()+'_'+selecteds[0].did+'_modify', title, path1);
				},
				verify : function() {
					var selecteds = this.grid.getSelecteds();
					if (selecteds.length == 0) {LG.tip(LG.INFO.ONE);return}
					...
					//审核信息
					var path1='TblContent/verify.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did;
					var title="审核内容信息";
					if(<%=dtype%>==1){
						//查看 问卷调查
						path1='TblContent/verifyquest.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did;
						title="审核问卷调查";
					}else if(<%=dtype%>==5){
						//查看 在线考试
						path1='TblContent/verifyexist.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did;
						title="审核在线考试";
					}else if(<%=dtype%>==15){
						//查看 培训内容, 可以关联考卷
						path1='TblContent/verify.do?dtype=<%=dtype%>&categoryType=<%=categoryType%>&IsView=1&menuno='+LG.getPageMenuNo() + '&id=' + selecteds[0].did;
						title="审核培训内容";
					}
