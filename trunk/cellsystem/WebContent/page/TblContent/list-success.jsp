			...
			var crGrid = {
				...
				init : function() {
					...
					this.grid = jQuery(self.main).ligerGrid({ 
			           ...
			           url:  'loadList.do?categoryType=<%=categoryType%>&type=<%=type%>&dtype=<%=dtype%>&content=<%=content%>', 
				...
				add : function() {
					...
					var pars=null;
					if('<%=type %>'!= ''){
						pars ={"query.did":<%=categoryType%>,"query.dstatus":10};
					}else {
						pars={"query.did":<%=categoryType%>};
					}
