			...
			<table id="detail" class="detail-tableSkin detail-tableSkinB">
				<tr>
					...
					<td style="width: 440px">
						<input class="EE" name="tblQuestionexam.dname"  validate="{'required':true}"/>
				...
				<tr>
					<th>考前必读：</th>
					<td colspan="3">
						<textarea style="width:700px" id="address" class="l-textarea" rows="4" cols="100" name='tblQuestionexam.dread' validate="{'required':true}"></textarea>
					</td>
				</tr>
			</table>
			...
			var crForm = {
				...
				save : function() {
					...
					mainFrm.ajaxForm({
						...
						success:function(data){
							//添加 在线考试  成功
							var examid=data.data.dmapping;// examid表示的是在线考试基本信息的id, 对应content表中的mapping
							var paperid=data.data.did;// contentid表示 在线考试内容表中的信息的 content表中的 id
							...
							//alert('ids:'+selectids+",sequences:"+sequences);
							//编辑 全部选项的顺序
								$.post(
				...
				search : function(search,grid) {
					...
					jQuery(search).ligerForm({
						...
						fields:[
						  ...
						  {display: "栏目设置", name:"dcategorytype",type: "radioList",width:380,
						  			options:{change:function(id){
						  				grid.loadServerData({'query.dcategorytype':id});
						  				},
						  					onRendered:function(){self.rendered(grid,this)},
											data:dt1,textField: 'name'}
						  },
