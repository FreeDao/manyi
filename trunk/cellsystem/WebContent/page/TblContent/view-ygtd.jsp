...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<link type="text/css" href="<%=path%>/static/xcloud/css/bottom.css" rel="stylesheet" />
		<script type="text/javascript" src="<%=path%>/static/xcloud/js/jquery.pikachoose.js"></script>	
	...
	<input type='hidden' value="<s:property value='#attr.ftpip' />" id='ftpip'/>
		<div class="detail-middle l-form" style="margin-left: 30px; padding-bottom:100px; height:700px;">
			...
			<div id="video" style="position:relative;z-index: 0;width:600px;height:400px;float: left;margin-left:20px;margin-bottom:30px;"><div id="a1"></div></div>
			<!--
			上面一行是播放器所在的容器名称，如果只调用flash播放器，可以只用<div id="a1"></div>
			-->	
			<div id='contentshow' style='margin-bottom:30px;'>
			...
			var crForm = {
				...
				callback : function(data) {
					...
					//图文
						$("#video").remove();
						$("#vod").remove();
						var vodurl ="http://"+$("#ftpip").val()+"/"+loadDt.dpicurl+"/";
						var str="<div style='font-size: 16px; font-weight: bold;'>标题: 手机用户上传内容</div><hr/>";
						str+="<div style='margin-top:15px;'>";
						var tmps =loadDt.dpicname;
						if(tmps != null && tmps != ''){
							if(tmps.substr(tmps.length-1,1) == ';'){
								tmps = tmps.substr(0,tmps.length-1);
							}
							tmps =tmps.split(";");
							/*
							if( tmps.length>1){
								str +='<div class="pikachoose">	<ul id="pikame"  class="jcarousel-skin-pika">';
								for(var j =0;j<tmps.length;j++){
									str +='<li><a href="javascript:void(0);" title=""><img src="'+vodurl+tmps[j]+'"/></a></li>';
								}
								str +='	</ul></div>';
							}else{
								str +='<center><img src="'+vodurl+tmps[0]+'"/></center>';
							}
							*/
							str +='<center><img src="'+vodurl+tmps[0]+'"/></center>';
						}
						str+="</div><div>"+loadDt.dcontent+"</div>";
						$("#contentshow").append($(str));
						/*
						if(tmps != null && tmps != '' && tmps.length>1){
							$("#pikame").PikaChoose({carousel:true,thumbOpacity:1});//设置多张图片
						}
						*/
